...
def run_full_llocv(valid_df, var_name, n_obs, rf_params, encoder=None, time_col="time"):
    """Full LLOCV for every (station, timestep). Returns DataFrame."""

    records = []
    for t, df_t in valid_df.groupby(time_col):
        ...
        if encoder is not None and "elev" in df_t.columns:
            ...
            if "clc_code" in df_t.columns:
                X_list.append(encoder.transform(df_t[["clc_code"]].values.reshape(-1, 1)).astype(np.float32))
        ...
        for i in range(n):
            ...
            mask[i] = False
            model = RFSI(n_obs=n_obs, rf_params=rf_params)
            model.fit(coords[mask], z_vals[mask], X_cov=X_cov[mask] if X_cov is not None else None)
            pred = model.predict(coords[[i]], X_cov_pred=X_cov[[i]] if X_cov is not None else None)
            records.append({
                "time": t,
