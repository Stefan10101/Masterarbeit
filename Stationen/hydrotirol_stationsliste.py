#!/usr/bin/env python3
"""
HYDRO_TIROL Station Metadata Collector (standalone)

Scans for LT*, N*, and SH* files and builds a station list
with correct measurement flags for temperature, precipitation and snow height.
"""
...
INPUT_DIR   = Path(DATA_ROOT / "hydro")
LOG_DIR     = Path(DATA_ROOT / "code" / "logs" / "hydro_tirol")
STATION_DIR = Path(DATA_ROOT / "stationen" / "hydro_tirol")
OUTPUT_JSON = STATION_DIR / "hydro_tirol_stations_with_measurement_flags.json"

LOG_DIR.mkdir(parents=True, exist_ok=True)
STATION_DIR.mkdir(parents=True, exist_ok=True)
LOG_FILE = LOG_DIR / "hydro_tirol_stations_metadata.log"
...
def get_variable_type(filename: str) -> str | None:
    """Detect variable type from filename prefix."""
    stem = filename.upper()
...
def main():
    ...
    log("=== HYDRO_TIROL Station Metadata Collector (all variables) ===")
    ...
    log(f"Found {len(all_files)} relevant files (LT, N, SH)")

    # Group by station name + collect measured variables
    ...
    for csv_path in all_files:
        ...
        # Store metadata only once (first successful parse)
    ...
    log(f"Saved {len(stations)} stations â†’ {OUTPUT_JSON}")

    # Ready-to-copy set
    ids = sorted(s["id"] for s in stations)
    print("\n" + "=" * 80)
    print("RELEVANT_HYDRO_TIROL_STATIONS = {")
    line = "    "
