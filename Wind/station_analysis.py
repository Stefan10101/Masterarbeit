...
# -*- coding: utf-8 -*-
"""
Master Thesis - Wind Full Analysis Plots (v2.2)
11 plots with 99th percentile per-station filtering + station map with counts in upper left
"""
...
# ==================== CONFIG ====================
BASE_DIR = Path(DATA_ROOT / "wind" / "aaadata" / "full_data_cold_seasons")
PLOTS_DIR = Path(DATA_ROOT / "plots" / "wind")
...
def load_and_clean_cold_season_data(base_dir: Path) -> pd.DataFrame:
    ...
    for json_path in json_files:
        try:
            ...
            # Wind-specific + 99th percentile per station
...
def plot_station_map(coords_df: pd.DataFrame, total_measurements: int):
    ...
    # Upper left corner: total stations and total measurements
    ...
    ax.set_title("Spatial Distribution of Wind Stations\n(Cold Seasons Data)", fontsize=14, pad=15)
...
def plot_A_overall_distribution(df: pd.DataFrame):
    ...
    mean_w = df["value"].mean()
    ax.axvline(mean_w, color='red', linestyle='--', linewidth=2, label=f'Mean: {mean_w:.1f} m/s')
    ax.set_xlabel('Wind speed (m/s)')
    ax.set_ylabel('Number of Observations')
    ax.set_title('A. Overall Wind Speed Distribution\n(All Cold-Season Data â€“ 99th percentile filtered)')
    ax.grid(True, alpha=0.3, axis='y')
    ax.legend()
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / "A_wind_distribution.png", dpi=350, bbox_inches='tight')
    logger.info("Saved A_wind_distribution.png")
    plt.close()
...
def plot_B_by_elevation_bands(df: pd.DataFrame):
    ...
    ax.set_xlabel('Wind speed (m/s)')
    ax.set_ylabel('Number of Observations')
    ax.set_title('B. Wind Speed Distribution by Elevation Bands')
    ax.grid(True, alpha=0.3, axis='y')
    ax.legend(title="Elevation Band")
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / "B_wind_by_elevation_bands.png", dpi=350, bbox_inches='tight')
    logger.info("Saved B_wind_by_elevation_bands.png")
    plt.close()

def plot_C_mean_vs_elevation(df: pd.DataFrame):
    station_stats = df.groupby("station").agg(mean_wind=("value", "mean"), hoehe=("hoehe", "first")).reset_index()
    fig, ax = plt.subplots(figsize=(10, 7))
    sc = ax.scatter(station_stats["hoehe"], station_stats["mean_wind"], c=station_stats["mean_wind"], cmap='Blues', s=60, edgecolor='black', alpha=0.9)
    z = np.polyfit(station_stats["hoehe"], station_stats["mean_wind"], 1)
    p = np.poly1d(z)
    x_line = np.linspace(station_stats["hoehe"].min(), station_stats["hoehe"].max(), 100)
    ax.plot(x_line, p(x_line), 'r--', linewidth=2.5, label=f'Trend: {z[0]:.4f} (m/s)/m')
    ax.set_xlabel('Elevation (hoehe) [m]')
    ax.set_ylabel('Mean Cold-Season Wind Speed (m/s)')
    ax.set_title('C. Mean Wind Speed vs Station Elevation')
    ax.grid(True, alpha=0.3)
    ax.legend()
    cbar = plt.colorbar(sc, ax=ax, shrink=0.75)
    cbar.set_label('Mean Wind Speed (m/s)')
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / "C_mean_wind_vs_elevation.png", dpi=350, bbox_inches='tight')
    logger.info("Saved C_mean_wind_vs_elevation.png")
    plt.close()
...
def plot_F_monthly_mean_by_elevation(df: pd.DataFrame):
    ...
    monthly = df.groupby(["month", "month_name", "hoehe"]).agg(mean_wind=("value", "mean")).reset_index()
    ...
    for band in ELEVATION_BANDS.keys():
        ...
        if not subset.empty:
            ax.plot(subset["month_name"], subset["mean_wind"], marker='o', linewidth=2.5, label=band)
    ax.set_xlabel('Month')
    ax.set_ylabel('Mean Wind Speed (m/s)')
    ax.set_title('F. Monthly Mean Wind Speed by Elevation Band')
    ax.grid(True, alpha=0.3)
    ax.legend(title="Elevation Band")
    plt.tight_layout()
...
def plot_I_variability_vs_elevation(df: pd.DataFrame):
    station_stats = df.groupby("station").agg(std_wind=("value", "std"), hoehe=("hoehe", "first")).reset_index()
    fig, ax = plt.subplots(figsize=(10, 7))
    sc = ax.scatter(station_stats["hoehe"], station_stats["std_wind"], c=station_stats["std_wind"], cmap='viridis', s=65, edgecolor='black', alpha=0.9)
    ax.set_xlabel('Elevation (hoehe) [m]')
    ax.set_ylabel('Wind Speed Standard Deviation (m/s)')
    ax.set_title('I. Wind Speed Variability vs Station Elevation')
    ax.grid(True, alpha=0.3)
    cbar = plt.colorbar(sc, ax=ax, shrink=0.75)
    cbar.set_label('Std Dev (m/s)')
    plt.tight_layout()
...
# ==================== MAIN ====================
def main():
    logger.info("=== Wind Full Analysis (11 plots) Start ===")
