#!/usr/bin/env python3
"""
Master Thesis - Wind Speed Data Pipeline (v3.0)
...
# ==================== HELPERS (identical across all pipelines) ====================
...
def read_station_json(json_path: Path) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        # === LIGHT QC for WIND ===
        unreasonable_mask = (df["value"] < 0) | (df["value"] > 80)
        n_unreasonable = unreasonable_mask.sum()
        if n_unreasonable > 0:
            df.loc[unreasonable_mask, "value"] = np.nan
            logger.info(f"QC | {network}/{station_raw} â†’ {n_unreasonable} values set to NaN ( <0 or >80 m/s )")
