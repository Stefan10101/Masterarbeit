...
# -*- coding: utf-8 -*-
r"""
Wind Speed QC Diagnostic Plotter - v3 (final adapted version)
==============================================================
All fixes from temperature version applied:
- Correct paths for new structure
- Proper file discovery (nested original + flat QC)
- Safe handling of empty periods
...
ORIGINAL_ROOT = Path(DATA_ROOT / "wind" / "data" / "data" / "full_2020-2025")
FINAL_ROOT    = Path(DATA_ROOT / "wind" / "paket" / "full_2020_2025_qc")
PLOT_DIR      = Path(DATA_ROOT / "plots" / "wind")
...
LOG_FILE = PLOT_DIR / f"wind_qc_diagnostic_v3_{datetime.now():%Y%m%d_%H%M%S}.log"
...
def plot_diagnostic(original_df, final_df, station, periods, out_path):
    ...
    ax1.set_title(f"QC Diagnostic â€“ {station} (Wind Speed)\n"
    ...
    if not periods.empty:
        ...
        ax2.set_title("Distribution of flagged period lengths (spike / icing / stuck / range)", fontsize=10)
...
def main():
    logger.info("=== WIND SPEED QC DIAGNOSTIC v3 START ===")
    ...
    # Original is nested, QC output is flat
    orig_files = {}
    for f in orig_dir.rglob("*_wind.parquet"):
