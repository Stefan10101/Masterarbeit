#!/usr/bin/env python3
""DATA_ROOT / "
hydro_vo unified data extractor (schneehoehe + niederschlag + temperatur)

objective
---------
single script that processes all three variable types from the hydro_vo csv exports
...
(the temperature script you confirmed is now working correctly).

- parse_csv_to_dict (especially the tz_localize("Europe/Vienna") block) 
  originates from readerMA.py lines ~92-105.
- This logic correctly handles real DST transitions (CET â†” CEST) used by HYDRO_VO.
- The original Schneehoehe_extract.py and Niederschlag_extract.py had 
  structurally identical code but the temperature version was the reference
  implementation after your debugging. By centralizing THIS version, any
  previous timestep/timezone drift is eliminated for all three variables.
...
LOG_FILE = LOG_DIR / f"hydrovo_extract_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def parse_csv_to_dict(input_path: Path):
    """
    Parse a single HYDRO_VO CSV (LT*, N* or SH*) into a dict.
    ...
    # This block is taken verbatim from the working readerMA.py implementation.
    data = []
    reading_data = False
    for line in lines:
        stripped = line.strip()
        ...
        if reading_data and ";" in line:
            ...
            if len(parts) == 2:
                ...
                # === CORRECT UTC CONVERSION (HYDRO_VO follows real DST) ===
...
def process_variable_type(
    ...
    """
    logging.info(f"=== {type_label} â€“ HYDRO_VO â†’ JSON (correct UTC) ===")
