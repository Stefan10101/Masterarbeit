...
# -*- coding: utf-8 -*-
r"""
Precipitation QC Diagnostic Plotter - v3 (final adapted version)
=================================================================
...
ORIGINAL_ROOT = Path(DATA_ROOT / "niederschlag" / "data" / "data" / "full_2020-2025")
FINAL_ROOT    = Path(DATA_ROOT / "niederschlag" / "paket" / "full_2020_2025_qc")
PLOT_DIR      = Path(DATA_ROOT / "plots" / "niederschlag")
...
LOG_FILE = PLOT_DIR / f"precip_qc_diagnostic_v3_{datetime.now():%Y%m%d_%H%M%S}.log"
...
def plot_diagnostic(original_df, final_df, station, periods, out_path):
    orig_cum = original_df["value"].fillna(0).cumsum()
    final_cum = final_df["value"].fillna(0).cumsum()

    fig, axes = plt.subplots(2, 1, figsize=(14, 9), gridspec_kw={'height_ratios': [3, 1]})
    ax1 = axes[0]
    ax1.plot(original_df["timestamp"], orig_cum, color="#1f77b4", linewidth=1.0, label="Original (pipeline)", alpha=0.9)
    ax1.plot(final_df["timestamp"], final_cum, color="#d62728", linewidth=1.1, label="Final (after QC)", alpha=0.95)
    for _, row in periods.iterrows():
        ax1.axvspan(row["start"], row["end"], alpha=0.22, color="red", label="Flagged by QC" if _ == 0 else "")

    removed_points = periods['n_points'].sum() if not periods.empty and 'n_points' in periods.columns else 0
    total_orig = orig_cum.iloc[-1] if len(orig_cum) > 0 else 0
    total_final = final_cum.iloc[-1] if len(final_cum) > 0 else 0
    removed_mm = total_orig - total_final

    ax1.set_title(f"QC Diagnostic â€“ {station} (Precipitation)\n"
                  f"Total accumulated: Original {total_orig:,.0f} mm | Final {total_final:,.0f} mm | "
                  f"Removed: {removed_mm:,.0f} mm in {len(periods)} periods ({removed_points} points)",
    ...
    if not periods.empty:
        ...
        ax2.set_title("Distribution of flagged period lengths (range / spike / stuck non-zero)", fontsize=10)
...
def plot_zoom(original_df, final_df, station, event_start, event_end, out_path):
    ...
    orig_zoom = original_df[(original_df["timestamp"] >= start) & (original_df["timestamp"] <= end)].copy()
    final_zoom = final_df[(final_df["timestamp"] >= start) & (final_df["timestamp"] <= end)].copy()
    orig_cum = orig_zoom["value"].fillna(0).cumsum()
    final_cum = final_zoom["value"].fillna(0).cumsum()

    fig, ax = plt.subplots(figsize=(13, 6))
    ax.plot(orig_zoom["timestamp"], orig_cum, color="#1f77b4", linewidth=1.5, label="Original (30-day accum)", alpha=0.9)
    ax.plot(final_zoom["timestamp"], final_cum, color="#d62728", linewidth=1.8, label="After QC (30-day accum)", alpha=0.95)
    ax.axvspan(event_start, event_end, alpha=0.28, color="red", label="Removed by QC")
    ax.set_ylabel("30-Day Accumulated Precipitation (mm)")
    ax.set_title(f"Zoom: {station} â€“ 30-day accumulated precip around flagged period\n"
                 f"{event_start.strftime('%Y-%m-%d %H:%M')} to {event_end.strftime('%Y-%m-%d %H:%M')} "
                 f"(Duration: {(event_end - event_start).total_seconds()/3600:.1f} h | Window: Â±{ZOOM_DAYS//2} days)",
                 fontsize=11)
...
def main():
    logger.info("=== PRECIPITATION QC DIAGNOSTIC v3 START ===")
    ...
    # Original is nested
    orig_files = {}
    for f in orig_dir.rglob("*_precip.parquet"):
