...
# -*- coding: utf-8 -*-
"""
Master Thesis - Precipitation Full Analysis Plots (v2.2)
- 6 detailed analysis plots (A-F, I) + 5 station overview plots (map, elevation, coverage, cov-vs-height, nearest-neighbor)
- 99th percentile per-station filtering to remove extreme outliers
...
# ==================== CONFIG ====================
BASE_DIR = Path(DATA_ROOT / "niederschlag" / "aaadata" / "full_data_cold_seasons")
PLOTS_DIR = Path(DATA_ROOT / "plots" / "niederschlag")
...
# ==================== DATA LOADING & CLEANING (99th percentile per station) ====================
def load_and_clean_cold_season_data(base_dir: Path) -> pd.DataFrame:
    all_data = []
    json_files = list(base_dir.rglob("*.json"))
    logger.info(f"Found {len(json_files)} JSON files. Loading and cleaning...")
    ...
    for json_path in json_files:
        try:
            ...
            # Precip-specific + 99th percentile per station (removes extreme outliers)
    ...
    logger.info(f"Loaded {len(df):,} valid records from {df['station'].nunique()} stations after 99th percentile filtering")
    logger.info(f"Date range: {df['timestamp'].min().date()} â†’ {df['timestamp'].max().date()}")
    logger.info(f"Precip range: {df['value'].min():.2f} mm â†’ {df['value'].max():.2f} mm")
    return df
...
def plot_station_map(coords_df: pd.DataFrame):
    ...
    ax.set_title("Spatial Distribution of Precipitation Stations\n(Cold Seasons)", fontsize=14, pad=15)
...
# ==================== DETAILED ANALYSIS PLOTS (A-F, I) ====================
def plot_A_overall_distribution(df: pd.DataFrame):
    fig, ax = plt.subplots(figsize=(10, 7))
    ax.hist(df["value"], bins=80, color='#1f77b4', edgecolor='black', alpha=0.85)
    mean_p = df["value"].mean()
    ax.axvline(mean_p, color='red', linestyle='--', linewidth=2, label=f'Mean: {mean_p:.2f} mm')
    ax.set_xlabel('Precipitation (mm)')
    ax.set_ylabel('Number of Observations')
    ax.set_title('A. Overall Precipitation Distribution\n(All Cold-Season Data â€“ 99th percentile filtered)')
    ax.grid(True, alpha=0.3, axis='y')
    ax.legend()
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / "A_precip_distribution.png", dpi=350, bbox_inches='tight')
    logger.info("Saved A_precip_distribution.png")
    plt.close()
...
def plot_B_by_elevation_bands(df: pd.DataFrame):
    ...
    ax.set_xlabel('Precipitation (mm)')
    ax.set_ylabel('Number of Observations')
    ax.set_title('B. Precipitation Distribution by Elevation Bands')
    ax.grid(True, alpha=0.3, axis='y')
    ax.legend(title="Elevation Band")
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / "B_precip_by_elevation_bands.png", dpi=350, bbox_inches='tight')
    logger.info("Saved B_precip_by_elevation_bands.png")
    plt.close()

def plot_C_mean_vs_elevation(df: pd.DataFrame):
    station_stats = df.groupby("station").agg(mean_precip=("value", "mean"), hoehe=("hoehe", "first")).reset_index()
    fig, ax = plt.subplots(figsize=(10, 7))
    sc = ax.scatter(station_stats["hoehe"], station_stats["mean_precip"], c=station_stats["mean_precip"], cmap='Blues', s=60, edgecolor='black', alpha=0.9)
    z = np.polyfit(station_stats["hoehe"], station_stats["mean_precip"], 1)
    p = np.poly1d(z)
    x_line = np.linspace(station_stats["hoehe"].min(), station_stats["hoehe"].max(), 100)
    ax.plot(x_line, p(x_line), 'r--', linewidth=2.5, label=f'Trend: {z[0]:.4f} mm/m')
    ax.set_xlabel('Elevation (hoehe) [m]')
    ax.set_ylabel('Mean Cold-Season Precipitation (mm)')
    ax.set_title('C. Mean Precipitation vs Station Elevation')
    ax.grid(True, alpha=0.3)
    ax.legend()
    cbar = plt.colorbar(sc, ax=ax, shrink=0.75)
    cbar.set_label('Mean Precipitation (mm)')
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / "C_mean_precip_vs_elevation.png", dpi=350, bbox_inches='tight')
    logger.info("Saved C_mean_precip_vs_elevation.png")
    plt.close()
...
def plot_E_example_time_series(df: pd.DataFrame):
    ...
    if len(stations) < 3:
        logger.warning("Not enough stations for example time series")
        return
...
def plot_F_monthly_mean_by_elevation(df: pd.DataFrame):
    ...
    monthly = df.groupby(["month", "month_name", "hoehe"]).agg(mean_precip=("value", "mean")).reset_index()
    ...
    for band in ELEVATION_BANDS.keys():
        ...
        if not subset.empty:
            ax.plot(subset["month_name"], subset["mean_precip"], marker='o', linewidth=2.5, label=band)
    ax.set_xlabel('Month')
    ax.set_ylabel('Mean Precipitation (mm)')
    ax.set_title('F. Monthly Mean Precipitation by Elevation Band')
    ax.grid(True, alpha=0.3)
    ax.legend(title="Elevation Band")
    plt.tight_layout()
...
def plot_I_variability_vs_elevation(df: pd.DataFrame):
    station_stats = df.groupby("station").agg(std_precip=("value", "std"), hoehe=("hoehe", "first")).reset_index()
    fig, ax = plt.subplots(figsize=(10, 7))
    sc = ax.scatter(station_stats["hoehe"], station_stats["std_precip"], c=station_stats["std_precip"], cmap='viridis', s=65, edgecolor='black', alpha=0.9)
    ax.set_xlabel('Elevation (hoehe) [m]')
    ax.set_ylabel('Precipitation Standard Deviation (mm)')
    ax.set_title('I. Precipitation Variability vs Station Elevation')
    ax.grid(True, alpha=0.3)
    cbar = plt.colorbar(sc, ax=ax, shrink=0.75)
...
# ==================== MAIN ====================
def main():
    logger.info("=== Precipitation Full Analysis (11 plots) Start ===")
    
    df = load_and_clean_cold_season_data(BASE_DIR)                    # detailed analysis data
    coords_df = collect_station_metadata(BASE_DIR)                    # station overview data
