#!/usr/bin/env python3
"""
Master Thesis - Precipitation Data Pipeline (v3.0)
Strict exact regrid (sum for precip), 30% coverage, new folder structure, thorough logging.
Special handling: HydroT/HydroVO different intervals â€“ summation captures all data.
"""
...
def regrid_to_30min_strict(df: pd.DataFrame, start: pd.Timestamp, end: pd.Timestamp, agg: str = "exact") -> pd.DataFrame:
    ...
    if agg == "sum":
        summed = df["value"].resample("30min", origin="start_day", label="right").sum(min_count=1)
...
# ==================== READ + LIGHT QC (Precipitation specific) ====================
...
def read_station_json(json_path: Path) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        # === LIGHT QC for PRECIPITATION ===
        unreasonable_mask = df["value"] < 0
        n_unreasonable = unreasonable_mask.sum()
        if n_unreasonable > 0:
            df.loc[unreasonable_mask, "value"] = np.nan
            logger.info(f"QC | {network}/{station_raw} â†’ {n_unreasonable} values set to NaN (negative precipitation)")
...
# ==================== PROCESS (identical, but uses sum regrid) ====================
...
def process_station(json_path: Path) -> Dict:
    ...
    # Precipitation always uses sum aggregation (captures all sub-interval data from HydroT/HydroVO)
    df_full = regrid_to_30min_strict(df, FULL_PERIOD_START, FULL_PERIOD_END, agg="sum")
...
def main():
    logger.info(f"=== {VARIABLE} PIPELINE START (strict sum regrid for precip, 30% threshold) ===")
